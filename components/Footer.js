  ...
  return (
    <footer className="bg-zelux-navy-light border-t border-zelux-gray-mid/30 mt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        <div>
          <span className="font-display text-2xl tracking-widest text-zelux-white glow-text">ZELUX</span>
          <p className="mt-4 text-xs text-zelux-gray leading-relaxed">Luxury redefined for the modern era. Curated fashion and technology for those who refuse to settle.</p>
          <a href="https://instagram.com/zelux.us" target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 text-xs text-zelux-cyan tracking-wider hover:text-zelux-cyan-light transition-colors duration-300 group">
        ...
        <div>
          ...
          <ul className="space-y-3">
            {['Apparel','Footwear','Electronics','New Arrivals','Bestsellers'].map(l => (
              <li key={l}><Link href={`/collections/${l.toLowerCase().replace(' ','-')}`} className="text-sm text-zelux-gray hover:text-zelux-cyan transition-colors duration-300">{l}</Link></li>
            ))}
          </ul>
        </div>
        <div>
        ...
        <div>
          ...
          <p className="text-sm text-zelux-gray leading-relaxed mb-3">All customer support is handled exclusively via Instagram.</p>
          <a href="https://instagram.com/zelux.us" target="_blank" rel="noreferrer" className="text-zelux-cyan text-sm hover:text-zelux-cyan-light transition-colors duration-300">@zelux.us &rarr;</a>
        </div>
      </div>
